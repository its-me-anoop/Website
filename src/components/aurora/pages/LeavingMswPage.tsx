    ...
    <Shell>
      ...
      <section id="packages" className="relative scroll-mt-24 py-24 sm:py-32">
        <div
          ...
          style={{ background: "linear-gradient(90deg,#6d5bff,#a6d93a)" }}
        />
        ...
        <Container>
          ...
          <ol className="relative mt-14 grid gap-5 md:grid-cols-4">
            <span
              ...
              style={{ background: "linear-gradient(90deg,#9b8cff,#45e3d4,#d4ff4f)" }}
            />
            {clearPath.steps.map((step, i) => (
              <Reveal as="li" key={step.title} delay={i * 0.08} className="relative">
                <span className="a-mono relative flex h-14 w-14 items-center justify-center rounded-full border border-a-line-2 bg-a-void text-[14px] text-a-lime">
                  0{i + 1}
                </span>
            ...
            {clearPath.dates.map((item, i) => (
              <Reveal as="li" key={item.label} delay={i * 0.08} className="h-full">
                <article className="a-glass a-spot flex h-full flex-col rounded-[26px] p-7">
                  <span className="flex items-center gap-2 text-a-cyan">
              ...
              <ul className="mt-7 space-y-3">
                {proofLinks.map((item) => (
                  <CheckItem key={item.href}>
                    <Link
                      href={item.href}
                      className="underline decoration-a-line-2 underline-offset-4 transition-colors hover:decoration-a-lime"
                    >
                      {item.label}
                    </Link>
                  </CheckItem>
