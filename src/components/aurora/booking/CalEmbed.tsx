...
/** Aurora lime, so the embed's selected dates and buttons match the site. */
const CAL_BRAND = "#ffb020";
...
export function CalEmbed({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      <div className="a-glass min-h-[560px] overflow-hidden rounded-[28px] p-2 sm:min-h-[600px]">
        <Cal
          namespace={NAMESPACE}
      ...
      <p className="mt-4 text-center text-[14px] leading-snug text-a-muted">
        Calendar not loading?{" "}
        <a
          href={site.booking.url}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1 text-a-ink underline decoration-a-line-2 underline-offset-4 transition-colors hover:decoration-a-amber"
        >
