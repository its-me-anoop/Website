...
/**
 * Domain-warped fractal noise folded into curtains of light. The
 * palette runs violet → blue → cyan → lime over a near-black night,
 * the pointer bends the field, and a vignette keeps the edges dark so
 * headline text always sits on a low-luminance ground.
 */
...
void main() {
  ...
  vec3 blue = vec3(0.24, 0.45, 1.0);
  vec3 cyan = vec3(0.18, 0.85, 0.80);
  vec3 lime = vec3(0.78, 1.0, 0.30);

  vec3 col = mix(violet, blue, smoothstep(0.1, 0.6, q.x));
  col = mix(col, cyan, smoothstep(0.35, 0.9, r.x));
  col = mix(col, lime, smoothstep(0.62, 1.0, r.y) * 0.55);

  float glow = curtain * (0.45 + 0.55 * uv.y) + pull * 0.18;
  vec3 night = vec3(0.012, 0.014, 0.032);
  vec3 outc = night + col * pow(max(glow, 0.0), 0.8) * 1.9;

  float vig = smoothstep(1.35, 0.2, length((uv - vec2(0.5, 0.6)) * vec2(1.0, 1.3)));
  outc *= mix(0.35, 1.0, vig);
  outc *= smoothstep(0.0, 0.3, uv.y) * 0.75 + 0.25;
  outc = min(outc, vec3(0.7));
