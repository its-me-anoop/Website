  ...
  return (
    <li className="flex items-start gap-3 py-3.5">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[15.5px] font-medium leading-snug text-a-ink">{check.title}</p>
        <p className="mt-1 max-w-[70ch] text-[14.5px] leading-[1.55] text-a-ink-soft">{check.detail}</p>
        {check.fix ? (
          <p className="mt-1.5 max-w-[70ch] text-[14.5px] leading-[1.55] text-a-ink">
        ...
        {check.evidence && check.evidence.length ? (
          <ul className="mt-2 flex flex-wrap gap-1.5">
            {check.evidence.map((e) => (
              <li
                key={e}
                className="max-w-full truncate rounded-[6px] bg-white/[0.06] px-2 py-1 font-mono text-[12px] text-a-ink-soft"
                title={e}
              >
                {e}
              </li>
...
export function CategoryRow({ category, index }: { category: CategoryScore; index: number }) {
  ...
  return (
    <Reveal as="li" delay={Math.min(index, 4) * 0.04} className="border-t border-a-line">
      <details className="group" open={index === 0 || counts.fail > 0}>
        <summary className="grid cursor-pointer list-none grid-cols-[auto_1fr_auto] items-start gap-x-4 gap-y-3 py-7 sm:grid-cols-[3rem_1fr_minmax(0,220px)_5rem] sm:items-center sm:gap-x-8 [&::-webkit-details-marker]:hidden">
          <span className="a-display text-[13px] tabular-nums text-a-amber sm:pt-0">0{index + 1}</span>
          <span className="min-w-0">
            <span className="a-display block text-[24px] leading-tight text-a-ink sm:text-[26px]">
              {category.name}
            </span>
            <span className="mt-1 block text-[14px] leading-snug text-a-ink-soft">
              {categoryMeta[category.id].blurb}
            </span>
          </span>
          <span className="col-start-2 sm:col-start-3">
            <span className="flex items-center justify-between text-[13px] text-a-muted">
          ...
          <span className={cn("row-start-1 col-start-3 justify-self-end a-display text-[40px] leading-none tabular-nums sm:col-start-4 sm:row-auto sm:text-[44px]", scoreTone(score))}>
            {score === null ? "–" : score}
          </span>
        </summary>
        <div className="pb-8 sm:pl-[calc(3rem+2rem)]">
          ...
          <ul className="divide-y divide-a-line border-t border-a-line">
...
export function CategoryList({ categories }: { categories: CategoryScore[] }) {
  return (
    <section id="categories" aria-labelledby="categories-heading" className="border-t border-a-line">
      <div className="mx-auto w-full max-w-[1280px] px-5 py-20 sm:px-8 sm:py-24">
        <Reveal className="max-w-[720px]">
          <Eyebrow className="text-a-muted">Seven areas</Eyebrow>
          <Display as="h2" id="categories-heading" size="md" className="mt-5 text-a-ink">
            How the site scores, area by <em>area</em>.
          </Display>
          <p className="mt-4 text-[16px] leading-[1.6] text-a-ink-soft">
        ...
        <ol className="mt-10 border-b border-a-line">
