    ...
    <section id="fix-first" aria-labelledby="fix-first-heading" className="">
      <div className="mx-auto w-full max-w-[1280px] px-5 py-20 sm:px-8 sm:py-28">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,320px)_1fr] lg:gap-16">
          <Reveal>
            <Label className="text-s-on-ink-2">Fix first</Label>
            <Display as="h2" id="fix-first-heading" size="md" className="mt-5 text-s-on-ink">
              {top.length === 0 ? (
                <>
            ...
            <p className="mt-5 text-[15.5px] leading-[1.6] text-s-on-ink-2">
              {top.length === 0
          ...
          {top.length > 0 ? (
            <ol className="grid">
              {top.map((check, i) => (
                <Reveal as="li" key={check.id} delay={i * 0.06} className="grid grid-cols-[2.5rem_1fr] gap-x-4 border-t border-s-line py-7 sm:grid-cols-[4rem_1fr] sm:gap-x-6">
                  <span className="s-display text-[40px] leading-none text-s-signal sm:text-[48px]">{i + 1}</span>
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
                      <h3 className="s-display text-[24px] leading-tight text-s-on-ink">{check.title}</h3>
                      <span className="inline-flex items-center gap-1.5 text-[12px] text-s-on-ink-2">
                        <StatusMark status={check.status} size={13} />
                        {impactLabel[check.impact]} &middot; {categoryMeta[check.category].name}
                      </span>
                    </div>
                    <p className="mt-3 max-w-[64ch] text-[15.5px] leading-[1.6] text-s-on-ink-2">{check.detail}</p>
                    {check.fix ? (
                      <p className="mt-2.5 max-w-[64ch] text-[15.5px] leading-[1.6] text-s-on-ink">
                        <span className="font-medium text-s-signal">Fix: </span>
                        {check.fix}
                      </p>
                    ) : null}
                  </div>
