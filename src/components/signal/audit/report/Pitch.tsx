...
import { ButtonLink, Display, Label } from "../../ui/primitives";
...
export function Pitch({ report }: { report: AuditReport }) {
  ...
  return (
    <section id="next-steps" aria-labelledby="next-steps-heading" className="border-t border-s-line">
      <div className="mx-auto w-full max-w-[1280px] px-5 py-20 sm:px-8 sm:py-28">
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
          <div>
            <Reveal>
              <Label className="text-s-on-ink-2">What Flutterly would do</Label>
              <Display as="h2" id="next-steps-heading" size="md" className="mt-5 text-s-on-ink">
                {keep ? (
                  <>
              ...
              <p className="mt-5 max-w-[560px] text-[16px] leading-[1.6] text-s-on-ink-2">
                {keep
            ...
            {!keep ? (
              <ol className="mt-10 grid">
                {weakest.map((cat, i) => (
                  <Reveal as="li" key={cat.id} delay={i * 0.06} className="grid grid-cols-[4.5rem_1fr] gap-x-5 border-t border-s-line py-6">
                    <span className={`s-display text-[40px] leading-none tabular-nums ${scoreTone(cat.score)}`}>
                      {cat.score}
                    </span>
                    <div>
                      <h3 className="s-display text-[22px] leading-tight text-s-on-ink">{cat.name}</h3>
                      <p className="mt-1.5 text-[14px] leading-[1.55] text-s-on-ink-2">
                        <span className="font-medium text-s-on-ink">Now:</span> {cat.summary}
                      </p>
                      <p className="mt-1.5 text-[14.5px] leading-[1.55] text-s-on-ink-2">
                        <span className="font-medium text-s-signal">A Flutterly build:</span> {promise[cat.id]}
                      </p>
                    </div>
                  </Reveal>
                ))}
            ...
            <Reveal delay={0.12} className="mt-10 border-t border-s-line pt-8">
              <h3 className="s-display text-[24px] text-s-on-ink">Next step: the full written audit, free.</h3>
              <p className="mt-3 max-w-[540px] text-[15.5px] leading-[1.6] text-s-on-ink-2">
              ...
              <p className="mt-4 text-[13.5px] text-s-on-ink-2">
          ...
          <div>
            <Reveal delay={0.08}>
              <Label className="text-s-on-ink-2">
                {keep ? "If a refresh is due" : "Suggested starting point"}
              </Label>
              <p className="mt-4 max-w-[440px] text-[15px] leading-[1.55] text-s-on-ink-2">
            ...
            <Reveal delay={0.16} className="mt-5">
              <ButtonLink href="/packages" tone="link" arrow="right">
        ...
        <Reveal delay={0.2} className="mt-16 border-t border-s-line pt-8">
          <p className="max-w-[760px] text-[14px] leading-[1.6] text-s-on-ink-2">
