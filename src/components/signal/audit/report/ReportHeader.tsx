...
import { Display, Label, Tag } from "../../ui/primitives";
  ...
  return (
    <header className="mx-auto w-full max-w-[1280px] px-5 sm:px-8">
      <div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-start lg:gap-16">
        <div>
          <Reveal>
            <Label className="text-s-on-ink-2">Instant website audit</Label>
            <Display as="h1" size="lg" className="mt-5 break-words text-s-on-ink">
              {page.host}
            </Display>
          </Reveal>
          <Reveal delay={0.06}>
            <p className="mt-4 text-[15px] leading-[1.55] text-s-on-ink-2">
              {page.title ? <>&ldquo;{page.title}&rdquo; &middot; </> : null}
              <a
                href={page.finalUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-s-signal underline-offset-4 hover:underline"
              >
          ...
          <Reveal delay={0.12}>
            <p className="mt-7 max-w-[560px] text-[19px] leading-[1.5] text-s-on-ink sm:text-[21px]">
              {report.verdict}
            </p>
          </Reveal>

          <Reveal delay={0.18} className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-3 text-[14px] text-s-on-ink-2">
            <form action="/audit" method="get" className="flex items-center gap-2">
              ...
              <select
                ...
                className="rounded-[8px] border border-s-line-2 bg-s-ink-2 px-2.5 py-1.5 text-[14px] text-s-on-ink"
              >
                {sectorOptions.map((o) => (
              ...
              <noscript>
                <button type="submit" className="rounded-[8px] bg-s-ink-2 px-3 py-1.5 text-s-on-ink">
                  Re-run
                </button>
              </noscript>
            </form>
            <button
              type="button"
              onClick={copyLink}
              className="inline-flex items-center gap-1.5 text-s-on-ink-2 underline-offset-4 hover:underline"
            >
        ...
        <Reveal delay={0.1} className="flex flex-col items-center lg:items-end">
          ...
          <p className="mt-4 flex items-center gap-2 text-[14px] text-s-on-ink-2">
            <span className="s-display text-[28px] leading-none text-s-on-ink">{report.grade}</span>
            <span>
      ...
      <Reveal delay={0.2} className="mt-14">
        <dl className="grid grid-cols-2 gap-x-6 gap-y-6 border-y border-s-line py-6 sm:grid-cols-3 lg:grid-cols-6">
          {facts.map((f) => (
            <div key={f.label}>
              <dt className="s-label text-s-on-ink-2">{f.label}</dt>
              <dd className={cn("s-display mt-2 text-[28px] leading-none text-s-on-ink tabular-nums")}>{f.value}</dd>
            </div>
          ))}
        </dl>
        <p className="mt-4 max-w-[760px] text-[13.5px] leading-[1.55] text-s-on-ink-2">
