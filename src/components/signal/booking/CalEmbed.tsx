...
/** Signal yellow, so the embed's selected dates and buttons match the site. */
const CAL_BRAND = "#ffd23f";
...
export function CalEmbed({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      <div className="min-h-[560px] overflow-hidden rounded-[4px] border border-s-line sm:min-h-[600px]">
        <Cal
          namespace={NAMESPACE}
      ...
      <p className="mt-4 text-center text-[14px] leading-snug text-s-on-ink-2">
        Calendar not loading?{" "}
        <a
          href={site.booking.url}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1 text-s-on-ink underline underline-offset-4 hover:decoration-s-signal"
        >
