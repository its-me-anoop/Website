...
/**
 * Primary nav for the dental demo. The one client component in the
...
export function DentalNav() {
  ...
  return (
    <nav aria-label="Primary" className="border-t border-white/15 bg-[var(--ddt-petrol-deep)]">
      <ul className="mx-auto grid w-full max-w-[1100px] grid-cols-2 sm:flex sm:flex-wrap sm:px-4">
        {navLinks.map((link) => {
          const active =
            link.href === "/demo/dental-practice"
              ? pathname === link.href
